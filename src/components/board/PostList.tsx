...
export function PostList({ posts, category }: { posts: any[], category: string }) {
  ...
  return (
    <div className="space-y-6">
      {posts.map((post) => (
        <article key={post.id} className="group bg-white rounded-2xl border border-slate-200 p-6 hover:shadow-md hover:border-emerald-200 transition-all flex flex-col">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              <Link href={`/profile/${post.author_id}`} className="w-10 h-10 rounded-full bg-slate-200 overflow-hidden flex-shrink-0 hover:opacity-80 transition-opacity">
              ...
              <div>
                <Link href={`/profile/${post.author_id}`} className="font-medium text-slate-900 hover:text-emerald-600 transition-colors">
          ...
          <Link href={`/board/${post.category || category}/${post.id}`} className="block">
            ...
            <p className="text-slate-500 leading-relaxed line-clamp-2 mb-4 text-sm">

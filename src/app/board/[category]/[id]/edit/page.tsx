...
export default function EditPostPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      setIsIndefinite(!post.recruitment_end_date && post.post_type === '같이 하자' && post.category === 'youmake')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (category === 'youmake') {
      // Allow author or admin to change it
    }
    ...
    const { error } = await supabase.from('posts').update({
      ...
      recruitment_end_date: (isIndefinite || status !== 'waiting') ? null : (recruitmentDate || null),
    }).eq('id', id)

...
export default async function BoardPage({ 
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10">
      <header className="mb-10 flex border-b border-slate-200 pb-6">
      ...
      <div className="flex gap-4 mb-6">
        <Link href={`/board/${category}?sort=recent`} className={`px-4 py-2 rounded-full font-medium text-sm transition-colors ${sort === 'created_at' ? 'bg-emerald-100 text-emerald-700' : 'text-slate-500 hover:bg-slate-100'}`}>최신순</Link>
        <Link href={`/board/${category}?sort=popular`} className={`px-4 py-2 rounded-full font-medium text-sm transition-colors ${sort === 'view_count' ? 'bg-emerald-100 text-emerald-700' : 'text-slate-500 hover:bg-slate-100'}`}>인기순</Link>
      </div>
